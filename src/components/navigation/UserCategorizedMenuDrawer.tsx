          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
            <div className="p-3 bg-neutral-900/80 border border-neutral-800 rounded-2xl flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <div className={`p-2 rounded-xl border ${currentUser ? 'bg-amber-500/20 border-amber-500/40 text-amber-400' : 'bg-neutral-800 border-neutral-700 text-neutral-400'}`}>
                  {currentUser ? <User className="w-4 h-4" /> : <LogIn className="w-4 h-4" />}
                </div>
                <div>
                  <p className="text-xs font-bold text-white">
                    {currentUser ? currentUser.name : 'Tamu / Pengunjung'}
                  </p>
                  <p className="text-[10px] text-neutral-400">
                    {currentUser ? `Status: ${currentUser.role.toUpperCase()}` : 'Belum Masuk Akun'}
                  </p>
                </div>
              </div>
              <button
