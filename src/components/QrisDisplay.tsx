...
export const QrisDisplay: React.FC<QrisDisplayProps> = ({
  ...
  return (
    <div className="space-y-4 max-w-md mx-auto">
      ...
      {/* INFORMASI PEMBAYARAN & PERINGATAN NOMINAL MANUAL */}
      <div className="bg-gradient-to-r from-amber-500/20 via-orange-500/20 to-red-500/20 border-2 border-amber-500/60 rounded-xl p-3.5 text-center shadow-lg space-y-1">
        <div className="flex items-center justify-center gap-1.5 text-amber-400 font-black text-xs uppercase tracking-wider">
          ...
          <span>PERINGATAN PEMBAYARAN QRIS RESMI</span>
        </div>
        <div className="text-base sm:text-lg font-black text-white leading-snug pt-1">
          Scan Dan Isi Nominal: <span className="text-amber-300 underline underline-offset-4">{feeAmount}</span>
        </div>
        <p className="text-[11px] text-neutral-300 font-mono pt-1">
          (Buka E-Wallet / Mobile Banking Anda, scan QR Code di bawah, lalu masukkan nominal transfer secara tepat)
        </p>
      </div>
      ...
      <div className={`bg-white rounded-2xl p-4 sm:p-5 text-slate-900 shadow-2xl border-4 ${meta.borderColor} space-y-4 relative overflow-hidden`}>
        ...
        <div className="relative group flex flex-col items-center justify-center bg-slate-50 p-3 rounded-2xl border-2 border-dashed border-slate-300">
          ...
          <div className="mt-3 bg-amber-50 border-2 border-amber-400 rounded-xl p-2.5 text-center">
            <p className="text-xs font-black text-amber-950 flex items-center justify-center gap-1">
              <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
              <span>Nominal yang harus dibayar: {feeAmount}</span>
            </p>
          </div>
        </div>
      ...
      <div className="bg-red-950/90 border-2 border-red-500 rounded-xl p-4 space-y-2 shadow-xl text-left">
        ...
        <p className="text-xs font-mono font-black text-amber-300 leading-relaxed bg-black/50 p-2.5 rounded-lg border border-amber-500/40">
          📌 Scan Dan Isi Nominal: {feeAmount} ({displayTitle})
        </p>
        <p className="text-xs font-mono font-extrabold text-white leading-relaxed bg-black/40 p-2.5 rounded-lg border border-red-500/40">
          ⏳ Setelah membayar dan menekan tombol konfirmasi, status Anda adalah: <strong className="text-amber-400">MENUNGGU KONFIRMASI ADMIN</strong>. Admin akan memeriksa bukti Anda dan menentukan status <strong className="text-emerald-400">SAH / DITOLAK</strong>.
        </p>
      </div>
        ...
        >
          ...
          >
            ...
            <div className="bg-amber-50 p-3 rounded-xl border border-amber-300 text-center space-y-1">
              <p className="text-xs font-black text-amber-950 flex items-center justify-center gap-1">
                <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
                <span>Nominal Pembayaran: {feeAmount}</span>
              </p>
              <p className="text-[11px] font-medium text-slate-700">
                📱 Arahkan kamera atau aplikasi E-Wallet / M-Banking Anda ke QR Code ini.
              </p>
            </div>
