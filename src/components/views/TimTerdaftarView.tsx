...
export const TimTerdaftarView: React.FC<TimTerdaftarViewProps> = ({ teams, onOpenRegisterModal }) => {
  ...
  // Hanya tim berstatus SAH yang ditampilkan di Direktori Tim Terdaftar
  const sahTeams = teams.filter(t => t.status === 'Sah');

  const filtered = sahTeams.filter((t) => {
    const matchesGame = selectedGameFilter === 'ALL' || t.game === selectedGameFilter;
    const matchesQuery = 
  ...
  const ffCount = sahTeams.filter(t => t.game === 'FF').length;
  const mlbbCount = sahTeams.filter(t => t.game === 'MLBB').length;
