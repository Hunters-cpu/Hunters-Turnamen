...
export const FormPendaftaranView: React.FC<FormPendaftaranViewProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const existingGameTeams = registeredTeams.filter(t => t.game === game);
    const nextSlot = existingGameTeams.length + 1;
    ...
    notifyAdminEvent(
      ...
      `Tim "${newTeam.teamName}" (${newTeam.game === 'FF' ? 'Free Fire' : 'Mobile Legends'}) mendaftar slot #${newTeam.slotNumber}. Kapten: ${newTeam.captainName} (${newTeam.captainPhone}).`,
      newTeam
    );
          ...
          <div className="bg-[#050505] p-4 rounded-2xl border border-amber-500/30 space-y-3 text-xs text-neutral-300 font-mono">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
              <div className="space-y-1 bg-[#0a0a0a] p-3 rounded-xl border border-neutral-800">
                ...
                <p>• <span className="text-neutral-400">Slot #:</span> <strong className="text-amber-400">#{submittedData.slotNumber}</strong></p>
      ...
      {!submittedData && (
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-[#0f0f0f] border border-neutral-800 p-5 rounded-2xl space-y-3 shadow-xl">
            <h3 className="text-xs font-black text-orange-400 uppercase tracking-wider flex items-center gap-2 border-b border-neutral-800 pb-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                type="button"
                onClick={() => setGame('FF')}
                className={`p-4 rounded-xl border font-black text-xs uppercase flex items-center justify-center gap-2 transition-all ${
                  game === 'FF'
              ...
              >
                ...
                <span>🔥 FREE FIRE (32 SLOT)</span>
              </button>

              <button
                type="button"
                onClick={() => setGame('MLBB')}
                className={`p-4 rounded-xl border font-black text-xs uppercase flex items-center justify-center gap-2 transition-all cursor-pointer ${
                  game === 'MLBB'
          ...
          <div className="bg-[#0f0f0f] border border-neutral-800 p-5 rounded-2xl space-y-4 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-neutral-800 pb-3">
              <h3 className="text-xs font-black text-emerald-400 uppercase tracking-wider flex items-center gap-2">
                ...
                <span>
                  3. PILIH METODE PEMBAYARAN BIAYA REGISTRASI (Rp {(game === 'FF' ? (paymentConfig.feeFf || 50000) : (paymentConfig.feeMlbb || 50000)).toLocaleString('id-ID')})
                </span>
              </h3>
            ...
            {paymentCategory === 'qris' && (
              <div className="bg-[#050505] p-4 rounded-xl border border-neutral-800 space-y-3 animate-in fade-in duration-200">
                ...
                <QrisDisplay 
                  ...
                  customFee={`Rp ${(game === 'FF' ? (paymentConfig.feeFf || 50000) : (paymentConfig.feeMlbb || 50000)).toLocaleString('id-ID')}`}
                />
              </div>
            )}
