...
— HUNTERS COMMUNITY by DEXZ STORE`;
    ...
    if (newStatus === 'Sah' && targetTeam) {
      ...
      if (botConfig && botConfig.autoSendVerifiedSah) {
        const messageText = (botConfig.templateSah || '')
          ...
          .replace('{SLOT}', String(targetTeam.slotNumber))
      ...
      showNotification(`Status tim "${teamName}" diubah menjadi SAH! 🤖 Bot WA otomatis mengirim teks verifikasi ke Kapten (${targetTeam.captainPhone})`);
    } else if (newStatus === 'Menunggu Pembayaran') {
      showNotification(`Status tim "${teamName}" diubah menjadi PENDING (Disembunyikan dari Daftar Tim Terdaftar)`);
    } else if (newStatus === 'Gagal') {
      showNotification(`Status tim "${teamName}" diubah menjadi GAGAL (Akan otomatis masuk ke menu Baru Saja Dihapus setelah 1 hari)`);
    }
  };
  ...
  const handleCreateNewTeam = (e: React.FormEvent) => {
    ...
    const existingInGame = registeredTeams.filter(t => t.game === newTeamGame);
    const nextSlot = existingInGame.length + 1;
    ...
    setNewCaptainPhone('');
    setNewRosterText('');
    showNotification(`Tim ${newTeam.teamName} berhasil ditambahkan manual!`);
  };
  ...
  const handleClearUpcomingTournaments = () => {
    if (
      !confirm(
        'Yakin ingin menghapus seluruh daftar turnamen yang sedang disiapkan dan belum dibuka pendaftarannya? Turnamen yang sudah berjalan TIDAK ikut terhapus.'
      )
    ) {
      return;
    }
    const activeOnly = (config.upcomingTournaments || []).filter(
      (t) => t.status === 'Pendaftaran Dibuka' || t.status === 'Slot Hampir Penuh'
    );
    handleSaveAllConfig({ ...config, upcomingTournaments: activeOnly }, 'Daftar turnamen mendatang yang belum dibuka berhasil dihapus!');
  };
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              <span>🗑️ KOSONGKAN SEMUA DATA (RESET)</span>
            </button>

            <div className="bg-[#050505] p-3.5 rounded-xl border border-neutral-800 text-right shrink-0">
          ...
          <div className="bg-[#0f0f0f] border border-amber-500/40 rounded-2xl p-6 space-y-5 shadow-2xl">
            ...
            <div className="p-5 bg-[#050505] border border-neutral-800 rounded-xl space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-[11px] font-bold text-neutral-300 block mb-1">URL / Link File Media Bukti:</label>
                  <input
                    type="text"
                    value={galleryMediaUrl}
                    onChange={(e) => setGalleryMediaUrl(e.target.value)}
                    placeholder="https://... (URL foto/video)"
                    className="w-full bg-[#0f0f0f] border border-neutral-800 rounded-lg p-2.5 text-xs text-cyan-300 focus:outline-none focus:border-amber-500 font-mono"
                  />
                </div>
              </div>
                  ...
                  {registeredTeams.map((team) => (
                    <tr key={team.id} className="hover:bg-neutral-900/50">
                      <td className="p-3 font-mono">
                        ...
                        <span className="text-[10px] text-neutral-500">Slot #{team.slotNumber}</span>
                      </td>
                      <td className="p-3">
                        <button
                          type="button"
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const newItem: UpcomingTournament = {
                    ...
                    prizePool: newUpcomingPrize.trim() || 'Sesuai Pendaftaran',
